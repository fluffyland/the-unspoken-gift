...
- **SUPABASE HAS MOVED EDGE FUNCTIONS TO THE NEW API KEY FORMAT.** The legacy `eyJhbG…`
  anon key still works for **PostgREST, Auth and Storage** (verified: `rest/v1` returns 200)
  but the Functions gateway now rejects it: `INVALID_API_KEY — expects sb_publishable_… or
  sb_secret_…`. So `create-login` will hit this too the next time anybody exercises it. The
  fix is the newer key from Project Settings → API Keys; whether to swap the client's key
  wholesale or use the new key only for function calls was still open at time of writing.
