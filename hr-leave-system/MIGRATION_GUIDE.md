...
| 16 | `migration_app_v38.sql` | **The Carry Forward figure on screen is now the one you can actually book.** Once the expiry date had passed the balance withheld those days but the screen still showed them — which also made the Annual Leave figure beside it go UP when leave was taken |
| 17 | `keepalive_ping_v3.sql` | The heartbeat that stops a free project sleeping, and expires due carry-over daily |
| 18 | `undo_year_start.sql` | Puts one "Start a new year" run back exactly as it was. Shows you what it would do before it does anything |
