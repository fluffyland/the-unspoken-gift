 ...
 *
 * Setup (once):
 *   1. Create a NEW Google Sheet (keep it separate from the Orders sheet).
 *   2. Extensions → Apps Script → paste this whole file → Save.
 *   3. Deploy → New deployment → type "Web app" → Execute as: Me,
 *      Who has access: Anyone → Deploy → copy the /exec URL.
 *   4. Send me that URL — I paste it into the site's ANALYTICS_ENDPOINT and redeploy.
 *   5. To view the dashboard: just open that /exec URL in your browser (bookmark it).
 *      Add ?range=1 / ?range=7 / ?range=30 to switch the time window (default 7 days).
 */
...
function doGet(e) {
  ...
  const days = ({ '1': 1, '7': 7, '30': 30 })[p.range] || 7;
  const agg = _aggregate(days);
  return HtmlService.createHtmlOutput(_render(agg, days))
    .setTitle('The Unspoken Gift — 访问漏斗')
    .addMetaTag('viewport', 'width=device-width, initial-scale=1');
}

function _aggregate(days) {
  const sh = _sheet();
  const last = sh.getLastRow();
  const cutoff = new Date().getTime() - days * 86400000;
  const stepSids = {};                 // event → Set(session)
  ...
  const steps = FUNNEL.map(f => ({ key: f[0], label: f[1], n: count(stepSids[f[0]]) }));
...
function _render(a, days) {
  ...
  a.steps.forEach((s, i) => {
    ...
    funnel +=
      ...
      '<div class="pct">' + pct + '%' +
      (i > 0 ? ' <span class="drop">▼' + drop + '%</span>' : '') + '</div></div>';
  });
  ...
  const rl = n => '<a class="rl' + (days === n ? ' on' : '') + '" href="?range=' + n + '">近 ' + n + ' 天</a>';
  return '<!doctype html><meta charset="utf-8"><style>' +
    ...
    '.ranges{margin:0 0 18px}.rl{display:inline-block;padding:6px 14px;border:1px solid var(--line);border-radius:100px;margin-right:8px;text-decoration:none;color:var(--gold);font-size:.82rem}.rl.on{background:var(--gold);color:#fff;border-color:var(--gold)}' +
    ...
    '<div class="ranges">' + rl(1) + rl(7) + rl(30) + '</div>' +
    '<div class="tiles">' +
    ...
    '<div class="tile"><div class="big">' + a.events + '</div><div class="cap">总事件数</div></div>' +
    '</div>' +
    '<div class="card"><h3>转化漏斗（百分比 = 占「进站」比例）</h3>' + (pv ? funnel : '<p style="color:#8a7a68;font-size:.85rem">这个时间段还没有数据 —— 上线埋点后回来看。</p>') + '</div>' +
    list('最受关注的场合', a.occ) +
    list('最受关注的礼盒', a.prod) +
    list('访客来源', a.ref) +
    list('设备', a.devices) +
