...
- ✅ **有效链接：https://leejianwei8313.github.io/the-unspoken-gift/**
- ❌ 不要再引用旧链接 `https://leejianwei8313.github.io/coding/`（已 404）。
  仓库已从 `coding` 改名为 `the-unspoken-gift`；git remote 仍叫 `coding`，
  靠 GitHub 自动跳转，`push` 照常生效，无需改 remote。
...
1. **每次改动新建递增编号文件** `theunspokengift_NN.html`（从最新一版复制），
   **绝不覆盖或替换旧文件**。当前最新为 `theunspokengift_21.html`。
